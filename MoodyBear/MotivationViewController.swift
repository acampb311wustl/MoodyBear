...
//        pieChartUpdate()
    ...
    func lineChartUpdate () {
        ...
        for i in 0 ..< currentLevels.count {
              avg = avg + Double(currentLevels[i].drive)
            yValues.append(ChartDataEntry(x: Double(i + 1), y: Double(currentLevels[i].drive)))
        }
        avg = avg/Double(currentLevels.count)
                  avg = round(100 * avg) / 100
             label.text = "Your average motivation level is \(avg) out of 5."
        let data = LineChartData()
        let ds = LineChartDataSet(entries: yValues)
           lineChart.chartDescription?.text = "Motivation Over Entries"
        data.addDataSet(ds)
        lineChart.data = data
